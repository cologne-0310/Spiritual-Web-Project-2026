...
const soulCards = {
    ...
    wealth: [
        { title: "豐盛意識", icon: "🌊", content: "金錢是能量的另一種形式。當您學會感恩已擁有的，更多的豐盛會隨之而來。" },
    ...
    oracleBtn.addEventListener('click', async () => {
        ...
        setTimeout(async () => {
            ...
            if (window.supabaseClient) {
                try {
                    await supabaseClient.from('oracle_drawings').insert([{
                        category: 'Crystal Ball',
                        card_title: 'Daily Guidance',
                        card_content: randomMsg
                    }]);
        ...
        if (window.supabaseClient) {
            try {
                await supabaseClient.from('oracle_drawings').insert([{
                    ...
                    card_content: randomCard.content
                }]);
            } catch (err) { console.error('Supabase log error:', err); }
        }
    }
